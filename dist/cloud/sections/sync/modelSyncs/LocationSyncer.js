...
const transferLocations_1 = require("../../../transferData/transferLocations");
const PermissionManager_1 = require("../../../../backgroundProcesses/PermissionManager");
const Log_1 = require("../../../../logging/Log");
class LocationSyncer extends SyncingBase_1.SyncingSphereItemBase {
    download() {
        return cloudAPI_1.CLOUD.forSphere(this.cloudSphereId).getLocations();
    }
    _getLocalData(store) {
    ...
    syncDown(locationsInState, locationsInCloud) {
        ...
        locationsInCloud.forEach((location_from_cloud) => {
            ...
            if (localId) {
                ...
                this.transferPromises.push(transferLocations_1.transferLocations.createLocal(this.actions, {
                    localId: localId,
        ...
        this.globalSphereMap.locations = Object.assign({}, this.globalSphereMap.locations, cloudIdMap);
        this.globalCloudIdMap.locations = Object.assign({}, this.globalCloudIdMap.locations, cloudIdMap);
    ...
    syncUp(store, locationsInState, localLocationIdsSynced) {
        ...
        localLocationIds.forEach((locationId) => {
            ...
            this.syncLocalLocationUp(store, location, locationId, localLocationIdsSynced[locationId] === true);
        });
    }
    syncLocalLocationUp(store, localLocation, localLocationId, hasSyncedDown = false) {
        ...
        if (!hasSyncedDown) {
            if (localLocation.config.cloudId) {
                ...
                if (!PermissionManager_1.Permissions.inSphere(this.localSphereId).canCreateLocations) {
                    return;
                }
                this.transferPromises.push(transferLocations_1.transferLocations.createOnCloud(this.actions, { localId: localLocationId, localSphereId: this.localSphereId, cloudSphereId: this.cloudSphereId, localData: localLocation })
                    .then((cloudId) => {
    ...
    _downloadLocationImage(localId, cloudId, imageId) {
        ...
        let toPath = Util_1.Util.getPath(localId + '.jpg');
        this.transferPromises.push(cloudAPI_1.CLOUD.forLocation(cloudId).downloadLocationPicture(toPath)
            .then((picturePath) => {
        ...
        }).catch((err) => { Log_1.LOGe.cloud("LocationSyncer: Could not download location picture to ", toPath, ' err:', err); }));
    }
    syncLocalLocationDown(localId, locationInState, location_from_cloud) {
        ...
        if (syncUtil_1.shouldUpdateInCloud(locationInState.config, location_from_cloud)) {
            if (!PermissionManager_1.Permissions.inSphere(this.localSphereId).canUploadLocations) {
                return;
            }
            this.transferPromises.push(transferLocations_1.transferLocations.updateOnCloud({
                localId: localId,
        ...
        else if (syncUtil_1.shouldUpdateLocally(locationInState.config, location_from_cloud)) {
            this.transferPromises.push(transferLocations_1.transferLocations.updateLocal(this.actions, {
                localId: localId,
                localSphereId: this.localSphereId,
                cloudData: location_from_cloud
            }).catch(() => { }));
...
exports.LocationSyncer = LocationSyncer;
//# sourceMappingURL=LocationSyncer.js.map
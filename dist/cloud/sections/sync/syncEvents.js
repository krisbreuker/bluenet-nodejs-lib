 ...
 */
exports.syncEvents = function (store) {
    ...
    return Promise.all(promises)
        ...
        .catch((err) => { Log_1.LOG.error('syncEvents: error', err); });
};
const handleRemove = function (state, events, actions) {
    ...
    messageIds.forEach((messageId) => {
        ...
        promises.push(cloudAPI_1.CLOUD.forSphere(payload.sphereId).deleteMessage(payload.cloudId)
            .then(() => { success(); })
            .catch((err) => {
            // already deleted
            if (err.status === 404) {
    ...
    scheduleIds.forEach((scheduleId) => {
        ...
        if (!eventData.cloudId) {
            return success();
        }
        if (!eventData.stoneId) {
            return success();
        }
        promises.push(cloudAPI_1.CLOUD.forStone(eventData.stoneId).deleteSchedule(eventData.cloudId)
            .then(() => { success(); })
            .catch((err) => {
            // already deleted
            if (err.status === 404) {
    ...
    messageIds.forEach((dbId) => {
        ...
        switch (payload.specialType) {
            case 'receivedMessage':
                promises.push(cloudAPI_1.CLOUD.receivedMessage(payload.cloudId).then(() => { success(); }).catch((err) => {
                ...
                break;
            case 'readMessage':
                promises.push(cloudAPI_1.CLOUD.readMessage(payload.cloudId).then(() => { success(); }).catch((err) => {
    ...
    userEventIds.forEach((userEventId) => {
        ...
        switch (payload.specialType) {
            case 'removeProfilePicture':
                promises.push(cloudAPI_1.CLOUD.removeProfileImage({ background: true }).then(() => { success(); })
                    .catch((err) => {
                    // even if there is no profile pic, 204 will be returned. Any other errors are.. errors?
                    Log_1.LOG.error("syncEvents Special: Could not remove image from cloud", err);
                }));
                break;
            case 'uploadProfilePicture':
                if (!state.user.picture) {
                    ...
                    promises.push(RNFS.exists(state.user.picture)
                        .then((fileExists) => {
                        if (fileExists === false) {
                            success();
                        }
                        else {
                            return cloudAPI_1.CLOUD.uploadProfileImage(state.user.picture);
                        }
                    })
                        .then(() => { success(); })
                        .catch((err) => {
                        Log_1.LOG.error("syncEvents Special: Could not upload image to cloud", err);
                    }));
                }
                break;
        }
    ...
    locationEventIds.forEach((locationEventId) => {
        ...
        switch (payload.specialType) {
            case 'removeLocationPicture':
                promises.push(cloudAPI_1.CLOUD.forLocation(payload.localId).deleteLocationPicture({ background: true }).then(() => { success(); })
                    .catch((err) => {
                    // even if there is no profile pic, 204 will be returned. Any other errors are.. errors?
                    Log_1.LOG.error("syncEvents Special: Could not remove location image from cloud", err);
                }));
                break;
            case 'uploadLocationPicture':
                if (!location.config.picture) {
                    ...
                    promises.push(RNFS.exists(location.config.picture)
                        .then((fileExists) => {
                        if (fileExists === false) {
                            success();
                        }
                        else {
                            return cloudAPI_1.CLOUD.forLocation(payload.localId).uploadLocationPicture(location.config.picture);
                        }
                    })
                        .then(() => { success(); })
                        .catch((err) => {
                        Log_1.LOG.error("syncEvents Special: Could not upload location image to cloud", err);
                    }));
                }
                break;
        }

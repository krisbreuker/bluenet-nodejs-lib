...
class SphereUserSyncer extends SyncingBase_1.SyncingSphereItemBase {
    downloadPendingInvites() {
        return cloudAPI_1.CLOUD.forSphere(this.cloudSphereId).getPendingInvites();
    }
    download() {
        return cloudAPI_1.CLOUD.forSphere(this.cloudSphereId).getUsers();
    }
    _getLocalData(store) {
        ...
        sphereUsersInCloud.forEach((sphere_user_in_cloud) => {
            ...
            let handleProfilePictureAndStore = (action) => {
                if (sphere_user_in_cloud.profilePicId) {
                    ...
                    this.transferPromises.push(cloudAPI_1.CLOUD.getUserPicture(this.cloudSphereId, sphere_user_in_cloud.email, sphere_user_in_cloud.id)
                        .then((filename) => {
                        ...
                        .catch((err) => {
                        Log_1.LOG.error("SphereUserSyncer: Failed getting user picture", err);
            ...
            if (sphereUsersInState[sphereUserId]) {
                ...
                if (sphereUserInState.firstName !== sphere_user_in_cloud.firstName ||
...
exports.SphereUserSyncer = SphereUserSyncer;
//# sourceMappingURL=SphereUserSyncer.js.map